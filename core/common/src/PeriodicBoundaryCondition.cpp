...
void PeriodicBoundaryCondition::extractBoxBoundaries(BoxInfo& boxInfo) {
    //Centered-Box Approach
    for (int j = 0; j < 3; ++j) {
        boxInfo.lb[j] = -boxInfo.boxSize[j] / 2.0;
        boxInfo.ub[j] = boxInfo.boxSize[j] / 2.0;
    }
...
//            // To calculate the center of the molecule
...
//            );
//
//            // To adjust the box so that its center matches the molecule's center
//            for (int i = 0; i < 3; ++i) {
...
//                boxInfo.ub[i] = boxInfo.boxSize[i] + boxInfo.lb[i];
//            }
//        }
