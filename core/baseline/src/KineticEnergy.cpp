...
void KineticEnergy::calculateKineticEnergy(const vector<Coords3D>& velocities, const vector<double>& invMass, const vector<double>& masses, const vector<Coords3D>& totalForces, double timeStep, int numAtoms, vector<double>& kineticEnergies, double& totalKEnergy) {
    ...
    for (size_t i = 0; i < numAtoms; ++i) {
        ...
        shiftedVelocity += { totalForces[i][0] * invMass[i]* timeShift, totalForces[i][1] * invMass[i]* timeShift, totalForces[i][2] * invMass[i]* timeShift };
